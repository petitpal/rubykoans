...
class Proxy
  attr_reader :messages_internal

  def initialize(target_object)
    @object = target_object
    @messages_internal = Hash.new(0)
  end

  def messages
    @messages_internal.keys
  end

  def called?(method_name)
    @messages_internal.keys.include?(method_name)
  end

  def number_of_times_called(method_name)
    @messages_internal[method_name.to_sym]
  end

  def method_missing(method_name, *args, &block)
    @messages_internal[method_name.to_sym] += 1
